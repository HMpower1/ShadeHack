...
import shade.features.hud.impl.TargetHud;
import shade.features.modules.Module;
import shade.features.modules.client.ClickGui;
import shade.features.modules.client.HudEditor;
import shade.gui.clickui.impl.*;
...
import shade.utility.render.animation.GearAnimation;
        ...
        if (hovered) {
            ...
            if (InputUtil.isKeyPressed(mc.getWindow().getHandle(), InputUtil.GLFW_KEY_DELETE) && button == 0) {
                DialogScreen dialogScreen = new DialogScreen(
                        ...
                        isPl() ? "PL PL" : "Reset module",
                        isPl() ? "PL PL PL PL " + module.getName() + "?" : "Are you sure you want to reset " + module.getName() + "?",
                        isPl() ? "Tak" : "Yes",
                        isPl() ? "Nie" : "No",
                        () -> {
                            if (module.isEnabled())
    ...
    public void keyTyped(int keyCode) {
        ...
        if (this.binding) {
            if (keyCode == GLFW.GLFW_KEY_ESCAPE || keyCode == GLFW.GLFW_KEY_DELETE) {
                ...
                Command.sendMessage((isPl() ? "PL PL PL PL " : "Removed bind from ") + module.getName());
            } else {
                module.setBind(keyCode, false, holdbind);
                Command.sendMessage(module.getName() + (isPl() ? " PL PL PL " : " bind changed to ") + module.getBind().getBind());
            }
            binding = false;
        }
    }

...
import static shade.core.manager.client.ConfigManager.IMAGES_FOLDER;
import static shade.features.modules.Module.mc;

public final class ExoUtility {
    ...
    public static @NotNull String getAuthors() {
        List<String> names = Shade.MOD_META.getAuthors()
                .stream()
                .map(Person::getName)
                .toList();
    ...
    public static void syncVersion() {
        try {
            if (!new BufferedReader(new InputStreamReader(new URL("https://raw.githubusercontent.com/Pan4ur/THRecodeUtil/main/syncVersion121.txt").openStream())).readLine().equals(Shade.VERSION))
                Shade.isOutdated = true;
    ...
    public static void parseStarGazer() {
        ...
        try {
            for (int page = 1; page <= 3; page++) {
                URL url = new URL("https://api.github.com/repos/Pan4ur/Shade-Recode/stargazers?per_page=100&page=" + page);
    ...
    public static void syncContributors() {
        try {
            ...
            int i = 0;
            while ((inputLine = in.readLine()) != null) {
                Shade.contributors[i] = inputLine.trim();
                i++;
            }
            in.close();
        } catch (Exception ignored) {
    ...
    public static void parseCommits() {
        try {
            URL url = new URL("https://api.github.com/repos/Pan4ur/Shade-Recode/commits?per_page=50");
            BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream(), StandardCharsets.UTF_8));

            changeLog.add("Changelog [Recode; Date: " + Shade.BUILD_DATE + "; GitHash:" + Shade.GITHUB_HASH + "]");
            changeLog.add("\n");

...
public class ConfigManager implements IManager {
    public static final String CONFIG_FOLDER_NAME = "SHADE";
    private static final String OLD_CONFIG_FOLDER_NAME = "ShadeRecode";
    ...
    private void migrateOldConfigFolder() {
        File oldFolder = new File(mc.runDirectory, OLD_CONFIG_FOLDER_NAME);
        if (!MAIN_FOLDER.exists() && oldFolder.exists()) {
            oldFolder.renameTo(MAIN_FOLDER);
        }
    }
    ...
    public void load(String name, String category) {
        File file = new File(CONFIGS_FOLDER, name + ".th");
        if (!file.exists()) {
            Command.sendMessage(isPl() ? "Konfiguracja " + name + " nie istnieje!" : "Config " + name + " does not exist!");
            return;
        }
    ...
    private void loadBinds(@NotNull File config) {
        ...
        try (FileReader reader = new FileReader(config, StandardCharsets.UTF_8)) {
            ...
            Command.sendMessage(isPl() ? "Wczytano bindy z konfiguracji: " + config.getName() : "Loaded bind from config: " + config.getName());
        } catch (IOException e) {
    ...
    public void loadCloud(String name) {
        Command.sendMessage(isPl() ? "Pobieram.." : "Downloading..");
        try (BufferedInputStream in = new BufferedInputStream(new URL("https://raw.githubusercontent.com/Pan4ur/THRecodeUtil/main/configs/" + name + ".th").openStream());
            ...
            Command.sendMessage(isPl() ? "Pobrano!" : "Downloaded!");
            load(name);
        } catch (Exception e) {
            Command.sendMessage(isPl() ? "Wystapil blad pobierania. Mozliwe, ze nazwa jest niepoprawna." : "There was an error downloading! Maybe the name is wrong?");
        }
    }
    ...
    public void loadModuleOnly(String name, Module module) {
        ...
        if (module.isEnabled()) {
            Shade.EVENT_BUS.unsubscribe(module);
        ...
        if (module.isEnabled())
            Shade.EVENT_BUS.subscribe(module);
    }
    ...
    private void load(@NotNull File config, String category) {
        ...
        try {
            ...
            Command.sendMessage(isPl() ? "Wczytano konfiguracje " + config.getName() : "Loaded " + config.getName());
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void loadModuleOnly(File config, Module module) {
        try (FileReader reader = new FileReader(config)) {
            ...
            Command.sendMessage(isPl() ? "Wczytano modul " + module.getName() + " z konfiguracji " + config.getName() :
    ...
    public void save(String name) {
        File file = new File(CONFIGS_FOLDER, name + ".th");
        if (file.exists()) {
            Command.sendMessage(isPl() ? "Nadpisuje " + name + "..." : "Overwriting " + name + "...");
            file.delete();
        } else {
            Command.sendMessage(isPl() ? "Konfiguracja " + name + " zostala zapisana!" : "Config " + name + " successfully saved!");
        }
        save(file);
    }
    ...
    private void parseModule(JsonObject object, String category) throws NullPointerException {
        ...
        for (Setting setting : module.getSettings()) {
            try {
                ...
                LogUtils.getLogger().warn("[Exohack] Module: " + module.getName() + " Setting: " + setting.getName() + " Error: ");
                e.printStackTrace();
            }
        }
    }

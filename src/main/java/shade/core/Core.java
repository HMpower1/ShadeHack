...
import shade.core.manager.client.MacroManager;
...
import static shade.features.modules.Module.fullNullCheck;
...
public final class Core {
    ...
    public void onTick(PlayerUpdateEvent event) {
        ...
        if (ModuleManager.clickGui.getBind().getKey() == -1) {
            ...
            Command.sendMessage(Formatting.RED + (isPl() ? "Gotowa konfiguracje mozesz pobrac komenda -> @cfg cloudlist." : "You can obtain a pre-built configuration by executing the following command -> @cfg cloudlist."));
        ...
        for (PlayerEntity p : mc.world.getPlayers()) {
            if (p.isDead() || p.getHealth() == 0)
                Shade.EVENT_BUS.post(new EventDeath(p));
        }
        ...
        if (autoSave.every(600000)) {
            ...
            Managers.NOTIFICATION.publicity("AutoSave", isPl() ? "Zapisuje konfiguracje.." : "Saving config..", 3, Notification.Type.INFO);
        }
    }
    ...
    public void drawGps(DrawContext e) {
        if (Shade.gps_position != null) {
            float dst = getDistance(Shade.gps_position);
            float xOffset = mc.getWindow().getScaledWidth() / 2f;
            float yOffset = mc.getWindow().getScaledHeight() / 2f;
            float yaw = getRotations(new Vec2f(Shade.gps_position.getX(), Shade.gps_position.getZ())) - mc.player.getYaw();

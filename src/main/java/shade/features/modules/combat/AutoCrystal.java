...
import shade.core.manager.world.CrystalManager;
...
import shade.utility.TickTimer;
import shade.utility.Timer;
import shade.utility.world.ExplosionUtility;
import shade.utility.math.MathUtility;
import shade.utility.math.PredictUtility;
...
public class AutoCrystal extends Module {
    ...
    private final Setting<Boolean> strictCenter = new Setting<>("CCStrict", true, v -> page.is(Pages.Place) && interact.getValue() == Interact.Strict);
    ...
    /*   RENDER   */
    public final Setting<Aura.ESP> targetEsp = new Setting<>("TargetESP", Aura.ESP.Shade, v -> page.is(Pages.Render));
    public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> targetEsp.is(Aura.ESP.ShadeV2) && page.is(Pages.Render));
    public final Setting<Integer> espLength = new Setting<>("ESPLength", 14, 1, 40, v -> targetEsp.is(Aura.ESP.ShadeV2) && page.is(Pages.Render)).addToGroup(espGroup);
    public final Setting<Integer> espFactor = new Setting<>("ESPFactor", 8, 1, 20, v -> targetEsp.is(Aura.ESP.ShadeV2) && page.is(Pages.Render)).addToGroup(espGroup);
    public final Setting<Float> espShaking = new Setting<>("ESPShaking", 1.8f, 1.5f, 10f, v -> targetEsp.is(Aura.ESP.ShadeV2) && page.is(Pages.Render)).addToGroup(espGroup);
    public final Setting<Float> espAmplitude = new Setting<>("ESPAmplitude", 3f, 0.1f, 8f, v -> targetEsp.is(Aura.ESP.ShadeV2) && page.is(Pages.Render)).addToGroup(espGroup);

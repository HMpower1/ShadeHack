...
import shade.utility.render.BlockAnimationUtility;
    ...
    public void onPostSync(EventPostSync e) {
        ...
        if (placeTimer.every(placeDelay.getValue()) && bestData != null && obbyResult.found() && isWorth()) {
            ...
            if (notification.getValue()) {
                String content;
                if (isPl())
                    content = "Stawiam na" + Formatting.GRAY + " X:" + bestData.position().getX() + " Y:" + bestData.position().getY() + " Z:" + bestData.position().getZ() + Formatting.WHITE + " obrazenia wzrosna o " + Formatting.RED + MathUtility.round2(bestData.damage - ModuleManager.autoCrystal.renderDamage);
                else

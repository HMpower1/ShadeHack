...
import shade.events.impl.EventPostSync;
...
public final class Burrow extends Module {
    ...
    public void onPostSync(EventPostSync e) {
        if (wait.getValue()) {
            ...
            if (!currentPos.equals(startPos)) {
                disable(isPl() ? "Wylaczono z powodu ruchu!" : "Disabled due to movement!");
                return;
            }
        }
        ...
        if (!mc.world.getBlockState(pos).isReplaceable()) {
            if (!wait.getValue())
                disable(isPl() ? "Nie mozna postawic bloku! Wylaczam.." : "Can't place the block! Disabling..");
            return;
        }
        ...
        for (Entity entity : mc.world.getNonSpectatingEntities(Entity.class, new Box(pos))) {
            if (entity != null && !mc.player.equals(entity)) {
                ...
                if (!wait.getValue())
                    disable(isPl() ? "Nie mozna postawic bloku! Wylaczam.." : "Can't place the block on! Disabling..");
                return;
            }
        }
    ...
    private void handleWeb(BlockPos pos) {
        ...
        if (!webResult.found()) {
            disable(isPl() ? "Nie PL!" : "No webs found!");
            return;
        }
        ...
        if (timer.passedMs(250)) {
            ...
            if (!wait.getValue() || placeDisable.getValue())
                disable(isPl() ? "Burrow wykonany! Wylaczam.." : "Successfully burrowed! Disabling..");
        }
    }
    ...
    private void handleSkull(BlockPos pos) {
        ...
        if (!skullResult.found()) {
            disable(isPl() ? "Nie PL!" : "No heads found!");
            return;
        }
    ...
    public void handleDefault(BlockPos pos) {
        ...
        if (!allowUp.getValue()) {
            ...
            if (upState.blocksMovement()) {
                if (!wait.getValue())
                    disable(isPl() ? "Blok nad glowa, nie mozna wykonac burrow! Wylaczam.." : "Above the head block, impossible to burrow! Disabling..");
                return;
            }
        }
        ...
        if (slot == -1) {
            disable(isPl() ? "Nie blokow!" : "No Block found!");
            return;
        }
        ...
        if (mc.isInSingleplayer()) {
            disable(isPl() ? "Jestes w trybie singleplayer.." : "Retard! You're in singleplayer..");
            return;
        }

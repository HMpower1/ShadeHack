...
import shade.features.modules.base.PlaceModule;
...
public final class Surround extends PlaceModule {
    ...
    public void onUpdate() {
        if ((mc.player.isDead() || !mc.player.isAlive() || mc.player.getHealth() + mc.player.getAbsorptionAmount() <= 0) && onDeath.getValue())
            disable(isPl() ? "Wylaczono z powodu smierci." : "Disable because you died.");
    }

    @SuppressWarnings("unused")
    @EventHandler(priority = EventPriority.HIGHEST)
    private void onTick(EventTick event) {
        if (prevY != mc.player.getY() && onYChange.getValue() && Shade.core.getSetBackTime() > 500) {
            disable(isPl() ? "Wylaczono z powodu zmiany Y!" : "Disabled due to Y change!");
            return;
        }
        ...
        if (!getBlockResult().found()) disable(isPl() ? "Nie blokow!" : "No blocks!");
    ...
    private void onPacketReceive(PacketEvent.@NotNull Receive event) {
        ...
        if (event.getPacket() instanceof PlayerPositionLookS2CPacket && onTp.getValue() == OnTpAction.Disable)
            disable(isPl() ? "Wylaczono z powodu rubberbandu!" : "Disabled due to a rubberband!");
    }

...
import shade.events.impl.PlayerUpdateEvent;
import shade.gui.notification.Notification;
import shade.injection.accesors.ILivingEntity;
...
import shade.utility.interfaces.IOtherClientPlayerEntity;
import shade.utility.math.MathUtility;
import shade.utility.player.InteractionUtility;
import shade.utility.player.InventoryUtility;
import shade.utility.player.PlayerUtility;
...
import shade.utility.render.animation.CaptureMark;
...
public class Aura extends Module {
    ...
    public final Setting<ESP> esp = new Setting<>("ESP", ESP.Shade);
    public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> esp.is(ESP.ShadeV2));
    public final Setting<Integer> espLength = new Setting<>("ESPLength", 14, 1, 40, v -> esp.is(ESP.ShadeV2)).addToGroup(espGroup);
    public final Setting<Integer> espFactor = new Setting<>("ESPFactor", 8, 1, 20, v -> esp.is(ESP.ShadeV2)).addToGroup(espGroup);
    public final Setting<Float> espShaking = new Setting<>("ESPShaking", 1.8f, 1.5f, 10f, v -> esp.is(ESP.ShadeV2)).addToGroup(espGroup);
    public final Setting<Float> espAmplitude = new Setting<>("ESPAmplitude", 3f, 0.1f, 8f, v -> esp.is(ESP.ShadeV2)).addToGroup(espGroup);
    ...
    public void handleKill() {
        if (target instanceof LivingEntity && (((LivingEntity) target).getHealth() <= 0 || ((LivingEntity) target).isDead()))
            Managers.NOTIFICATION.publicity("Aura", isPl() ? "Cel zostal zneutralizowany!" : "Target successfully neutralized!", 3, Notification.Type.SUCCESS);
    }
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        ...
        if (mc.player.isUsingItem() && pauseWhileEating.getValue())
            return;
        if(pauseBaritone.getValue() && Shade.baritone){
    ...
    public void onPacketReceive(PacketEvent.@NotNull Receive e) {
        if (e.getPacket() instanceof EntityStatusS2CPacket status)
            if (status.getStatus() == 30 && status.getEntity(mc.world) != null && target != null && status.getEntity(mc.world) == target)
                Managers.NOTIFICATION.publicity("Aura", isPl() ? ("Pomyslnie zniszczono tarcze gracza " + target.getName().getString()) : ("Succesfully destroyed " + target.getName().getString() + "'s shield"), 2, Notification.Type.SUCCESS);

        if (e.getPacket() instanceof PlayerPositionLookS2CPacket && tpDisable.getValue())
            disable(isPl() ? "Wylaczam z powodu teleportacji!" : "Disabling due to tp!");
        ...
        if (e.getPacket() instanceof EntityStatusS2CPacket pac && pac.getStatus() == 3 && pac.getEntity(mc.world) == mc.player && deathDisable.getValue())
            disable(isPl() ? "Wylaczam z powodu smierci!" : "Disabling due to death!");
    ...
    public float getAttackCooldownProgressPerTick() {
        return (float) (1.0 / mc.player.getAttributeValue(EntityAttributes.GENERIC_ATTACK_SPEED) * (20.0 * Shade.TICK_TIMER * (tpsSync.getValue() ? Managers.SERVER.getTPSFactor() : 1f)));
    }
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case ShadeV2 -> Render3DEngine.renderGhosts(espLength.getValue(), espFactor.getValue(), espShaking.getValue(), espAmplitude.getValue(), target);
            case Shade -> Render3DEngine.drawTargetEsp(stack, target);
        }
    ...
    /*
     * Algorytm oparty o zachowanie podobne do logo DVD.
     * Punkt porusza sie wewnatrz hitboksa celu, odbija sie od jego granic
     * i wybiera osiagalna pozycje celowania dla bardziej naturalnych rotacji.
     */
    ...
    public enum ESP {
        Off, Shade, NurikZapen, CelkaPasta, ShadeV2
    }

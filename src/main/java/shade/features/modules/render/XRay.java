...
import shade.utility.math.FrameRateCounter;
...
public class XRay extends Module {
        ...
        if (brutForce.getValue()) {
            if (all != done) {
                ...
                if (mc.player.age % 8 == 0 && MovementUtility.isMoving())
                    sendMessage(isPl() ? "Nie ruszaj sie podczas deobfuskacji!" : "Don't move while deobf!");
            } else {
                Box newArea = getArea();
    ...
    public void onRender3D(MatrixStack stack) { //FABOS IDI NAHUI
        ...
        if (mc.isInSingleplayer()) {
            disable(isPl() ? "Jestes w singleplayerze" : "Bro, you're in singleplayer");
            return;
        }

        if (mc.player.getMainHandStack().getItem() instanceof PickaxeItem) {
            if (mc.player.age % 8 == 0) disable(isPl() ? "Zdejmij kilof z reki!" : "Remove pickaxe from ur hand!");
            return;
        }
    ...
    public void onRender2D(DrawContext context) {
        if (brutForce.getValue()) {
            ...
            FontRenderers.sf_bold.drawCenteredString(context.getMatrices(), done + " / " + all + (isPl() ? " Zostalo: " : " Estimated time: ") + time + "s", posX + 68, posY + 18, -1);

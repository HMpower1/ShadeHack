...
import shade.injection.accesors.IInteractionManager;
import shade.features.modules.Module;
import shade.setting.Setting;
import shade.utility.interfaces.IEntity;
...
import static shade.utility.player.MovementUtility.isMoving;
...
public class Speed extends Module {
    ...
    private shade.utility.Timer elytraDelay = new shade.utility.Timer();
    private shade.utility.Timer startDelay = new shade.utility.Timer();
    ...
    public void onSync(EventSync e) {
        ...
        if (mode.getValue() == Mode.MatrixJB) {
            ...
            if (MovementUtility.isMoving() && closeToGround && mc.player.fallDistance <= 0) {
                ...
                Shade.TICK_TIMER = matrixJBSpeed.getValue();
    ...
    public void modifyVelocity(EventPlayerTravel e) {
        if (mode.getValue() == Mode.GrimEntity && !e.isPre() && Shade.core.getSetBackTime() > 1000) {
            for (PlayerEntity ent : Managers.ASYNC.getAsyncPlayers()) {
                if (ent != mc.player && mc.player.squaredDistanceTo(ent) <= 2.25) {
                    float p = mc.world.getBlockState(((IEntity) mc.player).exoHack_Recode$getVelocityBP()).getBlock().getSlipperiness();
        ...
        if ((mode.is(Mode.GrimEntity2) || mode.is(Mode.GrimCombo)) && !e.isPre() && Shade.core.getSetBackTime() > 1000 && MovementUtility.isMoving()) {
            int collisions = 0;
    ...
    public void onTick(EventTick e) {
        ...
        if ((mode.is(Mode.GrimIce) || mode.is(Mode.GrimCombo)) && mc.player.isOnGround()) {
            BlockPos pos = ((IEntity) mc.player).exoHack_Recode$getVelocityBP();
    ...
    public void onUpdate() {
        ...
        if (mode.getValue() == Mode.ElytraLowHop) {
            ...
            if (mc.world.getBlockCollisions(mc.player, mc.player.getBoundingBox().expand(-0.29, 0, -0.29).offset(0.0, -3, 0.0f)).iterator().hasNext() && elytraDelay.passedMs(150) && startDelay.passedMs(500)) {
                ...
                if (elytra == -1) disable(isPl() ? "Ten tryb wymaga elytry!" : "You need elytra for this mode!");
    ...
    public void onMove(EventMove event) {
        ...
        if (MovementUtility.isMoving()) {
            Shade.TICK_TIMER = useTimer.getValue() ? 1.088f : 1f;

...
import shade.features.modules.combat.Criticals;
...
import static shade.features.modules.player.ElytraSwap.getChestPlateSlot;
...
public class ElytraPlus extends Module {
    ...
    private final shade.utility.Timer startTimer = new shade.utility.Timer();
    private final shade.utility.Timer redeployTimer = new shade.utility.Timer();
    private final shade.utility.Timer strictTimer = new shade.utility.Timer();
    private final shade.utility.Timer pingTimer = new shade.utility.Timer();
    ...
    public void onEnable() {
        if (mc.player.getY() < infiniteMaxHeight.getValue() && mode.getValue() == Mode.Pitch40Infinite) {
            disable(
                    isPl() ?
                            "Wzleć powyzej " + Formatting.AQUA + infiniteMaxHeight.getValue() + Formatting.GRAY + " wysokosci!" :
    ...
    private void doPitch40Infinite() {
        ...
        if (is.isOf(Items.ELYTRA)) {
            ...
            if (is.getDamage() > 380 && mc.player.age % 100 == 0) {
                Managers.NOTIFICATION.publicity("Elytra+", isPl() ? "Elytra zaraz sie zepsuje!" : "Elytra's about to break!", 2, Notification.Type.WARNING);
    ...
    public void onPacketSend(PacketEvent.SendPost event) {
        ...
        if (event.getPacket() instanceof PlayerInteractEntityC2SPacket p && mode.is(Mode.FireWork) && grim.getValue().isEnabled() && fireWorkExtender.getValue())
            if (flying && flightZonePos != null && Criticals.getEntity(p).age < (pingTimer.getPassedTimeMs() / 50f))
                sendMessage(Formatting.RED + (isPl() ? "W tym trybie nie mozna atakowac bytow, ktore pojawily sie po wlaczeniu modulu!" : "In this mode, you cannot hit entities that spawned after the module was turned on!"));
    }
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        ...
        if (e.getPacket() instanceof PlayerPositionLookS2CPacket) {
            ...
            if (disableOnFlag.getValue() && mode.is(Mode.FireWork))
                disable(isPl() ? "Wylaczono z powodu flagi!" : "Disabled due to flag!");
        }
        ...
        if (mode.is(Mode.FireWork) && grim.getValue().isEnabled() && fireWorkExtender.getValue() && flying) {
            if (!pingTimer.passedMs(50000)) {
                if (pingTimer.passedMs(1000)) {
                    ...
                    FontRenderers.sf_bold.drawCenteredString(context.getMatrices(), isPl() ? ("Zostalo " + timeS + " sekund i " + dist + " metrow") : (timeS + " seconds and " + dist + " meters left"),
    ...
    private void noFireworks() {
        disable(isPl() ? "NiePL PL PL PL!" : "No fireworks in the hotbar!");
        flying = false;
    }

    private void noElytra() {
        disable(isPl() ? "NiePL PL PL PL!" : "No elytras found in the inventory!");
        flying = false;
    }
    ...
    private int getFireworks() {
        ...
        if (firesInHotbar == -1) {
            if (!allowFireSwap.getValue()) {
                disable(isPl() ? "Nie PL!" : "No fireworks!");

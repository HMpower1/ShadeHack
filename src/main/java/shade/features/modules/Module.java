package shade.features.modules;
...
import shade.core.manager.client.CommandManager;
...
import shade.features.modules.client.Windows;
import shade.features.modules.client.UnHook;
...
import static shade.Shade.LOGGER;
import static shade.core.Managers.NOTIFICATION;
...
public abstract class Module {
    ...
    protected void sendPacketSilent(Packet<?> packet) {
        if (mc.getNetworkHandler() == null) return;
        Shade.core.silentPackets.add(packet);
        mc.getNetworkHandler().sendPacket(packet);
    }
    ...
    public void enable() {
        ...
        if (isOn()) Shade.EVENT_BUS.subscribe(this);
        if (fullNullCheck()) return;

        LogUtils.getLogger().info("[Shade] enabled " + this.getName());
        Managers.MODULE.sortModules();

        if (!ignoreSoundList.contains(getDisplayName())) {
            NOTIFICATION.publicity(getDisplayName(), isPl() ? "Modul PL!" : "Was Enabled!", 2, Notification.Type.ENABLED);
    ...
    public void disable() {
        try {
            Shade.EVENT_BUS.unsubscribe(this);
        ...
        LOGGER.info("[Shade] disabled {}", getName());

        if (!ignoreSoundList.contains(getDisplayName())) {
            NOTIFICATION.publicity(getDisplayName(), isPl() ? "Modul PL!" : "Was Disabled!", 2, Notification.Type.DISABLED);

...
public class ServerHelper extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.getPacket() instanceof GameMessageS2CPacket pac) {
            if (spek.getValue()) {
                ...
                if (content.contains("spec") || content.contains("spec") || content.contains("spec") || content.contains("spek") || content.contains("spec")) {
                    String name = ExoUtility.solveName(pac.content().getString());
                    Managers.NOTIFICATION.publicity("SpekNotification", isPl() ? name + " chce, aby go sledzono" : name + " wants to be followed", 3, Notification.Type.WARNING);
                }
            }

            if (photomath.getValue())
                if (pac.content().getString().contains("Rozwiaz: ") && Objects.equals(ExoUtility.solveName(pac.content().getString()), "FATAL ERROR"))
                    try {
                        mc.player.networkHandler.sendChatMessage(String.valueOf(Integer.parseInt(StringUtils.substringBetween(pac.content().getString(), "Rozwiaz: ", " + ")) + Integer.parseInt(StringUtils.substringBetween(pac.content().getString(), " + ", " kto pierwszy"))));
                    } catch (Exception ignored) {
                    }

            if (antiTpHere.getValue()) {
                if (pac.content().getString().contains("Teleportacja...") && check(pac.content().getString())) {
                    flag = true;
                    atphtimer.reset();
                }
            }

            if (airDropWay.getValue() && pac.content().getString().contains("Airdrop")) {
                try {
                    int xCord = Integer.parseInt(StringUtils.substringBetween(pac.content().getString(), "koordynaty X: ", " Y:"));
                    ...
                    Shade.gps_position = new BlockPos(xCord, yCord, zCord);
                    Managers.NOTIFICATION.publicity("FGHelper", "Ustawiono waypoint na airdrop! X: " + xCord + " Y: " + yCord + " Z: " + zCord, 5, Notification.Type.SUCCESS);
                } catch (Exception ignored) {
                }
            }
        }
    ...
    public void onUpdate() {
        if (flag && atphtimer.passedMs(100) && antiTpHere.getValue()) {
            StringBuilder log = new StringBuilder("Teleportowano cie na X: " + (int) mc.player.getX() + " Z: " + (int) mc.player.getZ() + ". Najblizsi gracze: ");
        ...
        if (mc.player.currentScreenHandler instanceof GenericContainerScreenHandler chest && aucHelper.getValue()) {
            if (mc.currentScreen.getTitle().getString().contains("Aukcja") || mc.currentScreen.getTitle().getString().contains("Szukaj")) {
    ...
    public boolean check(String checkstring) {
        return checktimer.passedMs(3000) && (Objects.equals(ExoUtility.solveName(checkstring), "FATAL ERROR"));
    }

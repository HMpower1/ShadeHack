...
public class AutoFlyme extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        if (e.getPacket() instanceof GameMessageS2CPacket) {
            ...
            if ((packet.content().getString().contains("PL PL PL") || packet.content().getString().contains("PL PL PL usunietyPL")) && timer.passedMs(1000)) {
                mc.player.networkHandler.sendChatCommand("flyme");
                mc.player.networkHandler.sendChatCommand("flyme");
                timer.reset();
            }
    ...
    public void onUpdate() {
        if (useTimer.getValue()) Shade.TICK_TIMER = 1.088f;

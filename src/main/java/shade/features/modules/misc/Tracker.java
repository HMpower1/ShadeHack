...
import shade.events.impl.EventEntitySpawn;
...
public class Tracker extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        if (event.getPacket() instanceof GameMessageS2CPacket pac) {
            ...
            if (!s.contains("<") && (s.contains("has accepted your duel request") || s.contains("Accepted the duel request from"))) {
                sendMessage(isPl() ? "Pojedynek zaakceptowany! Odwieżam cel..." : "Duel accepted! Resetting target...");
                trackedPlayer = null;
                awaitingExp.set(0);
                crystals.set(0);
                exp.set(0);
    ...
    public void onUpdate() {
        ...
        for (PlayerEntity player : mc.world.getPlayers()) {
            ...
            if (found && only1v1.getValue()) {
                disable(isPl() ? "Nie jestes w pojedynku! Wylaczam.." : "Disabled, you are not in a 1v1! Disabling...");
                return;
            }
            if (trackedPlayer == null)
                sendMessage(Formatting.LIGHT_PURPLE + (isPl() ? "Sledze " : "Now tracking ") + Formatting.DARK_PURPLE + player.getName().getString() + Formatting.LIGHT_PURPLE + "!");
        ...
        if (expStacks != exp) {
            expStacks = exp;
            if (isPl()) sendMessage(Formatting.DARK_PURPLE + trackedPlayer.getName().getString() + Formatting.LIGHT_PURPLE + " uzyl " + Formatting.WHITE + exp + Formatting.LIGHT_PURPLE + (exp == 1 ? " stack" : " stackPL") + " butelek doswiadczenia!");
        ...
        if (crystalStacks != crystals) {
            ...
            if (!isPl()) sendMessage(Formatting.DARK_PURPLE + trackedPlayer.getName().getString() + Formatting.LIGHT_PURPLE + " used " + Formatting.WHITE + crystals + Formatting.LIGHT_PURPLE + (crystals == 1 ? " stack" : " stacks") + " of Crystals!");
            else sendMessage(Formatting.DARK_PURPLE + trackedPlayer.getName().getString() + Formatting.LIGHT_PURPLE + " uzyl " + Formatting.WHITE + crystals + Formatting.LIGHT_PURPLE + (crystals == 1 ? " stack" : " stackPL") + " krysztalow!");
        }
    }
    ...
    public void sendTrack() {
        if (trackedPlayer != null) {
            ...
            if (isPl()) builder = new StringBuilder().append(trackedPlayer.getName().getString()).append(Formatting.LIGHT_PURPLE).append(" uzyl ").append(Formatting.WHITE).append(c).append(Formatting.LIGHT_PURPLE).append(" (").append(Formatting.WHITE);
            ...
            if (isPl()) builder.append(Formatting.LIGHT_PURPLE).append(") krysztalow i ").append(Formatting.WHITE).append(e).append(Formatting.LIGHT_PURPLE).append(" (").append(Formatting.WHITE);
            ...
            if (isPl()) builder.append(Formatting.LIGHT_PURPLE).append(") butelek doswiadczenia.");

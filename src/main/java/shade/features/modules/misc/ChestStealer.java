...
import static shade.features.modules.render.StorageEsp.getBlockEntities;
        ...
        if (mc.player.currentScreenHandler instanceof GenericContainerScreenHandler chest) {
            for (int i = 0; i < chest.getInventory().size(); i++) {
                ...
                if (slot.hasStack() && isAllowed(slot.getStack())
                        ...
                        && !(mc.currentScreen.getTitle().getString().contains("Aukcja") || mc.currentScreen.getTitle().getString().contains("zakupy"))) {

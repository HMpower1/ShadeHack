...
import shade.core.manager.client.AsyncManager;
            ...
            if (timer.every(800)) {
                if (mc.crosshairTarget instanceof EntityHitResult ehr && ehr.getEntity() instanceof PlayerEntity)
                    if (Managers.FRIEND.isFriend(ehr.getEntity().getName().getString())) {
                        ...
                        Command.sendMessage(isPl() ? "§b" + ehr.getEntity().getName().getString() + "§r usuniety PL PL!" : "§b" + ehr.getEntity().getName().getString() + "§r removed from friends!");
                    } else {
                        Managers.FRIEND.addFriend(ehr.getEntity().getName().getString());
                        Command.sendMessage(isPl() ? "Dodano znajomego §b" + ehr.getEntity().getName().getString() : "Added friend §b" + ehr.getEntity().getName().getString());
                    }
            }
        }),

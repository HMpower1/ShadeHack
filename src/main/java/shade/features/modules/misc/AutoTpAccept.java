...
public class AutoTpAccept extends Module {
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof GameMessageS2CPacket) {
            ...
            if (packet.content().getString().contains("PL") || packet.content().getString().contains("tpaccept")) {
                if (onlyFriends.getValue()) {
                    if (Managers.FRIEND.isFriend(ExoUtility.solveName(packet.content().getString()))) {
    ...
    public void onRender2D(DrawContext context) {
        if (duo.getValue() && tpTask != null) {
            String text = (isPl() ? "PL PL " : "Awaiting target ") + MathUtility.round((timeOut.getValue() * 1000 - (System.currentTimeMillis() - tpTask.time())) / 1000f, 1);
    ...
    public void acceptRequest(String name) {
        if (grief.getValue()) mc.getNetworkHandler().sendChatCommand("tpaccept " + ExoUtility.solveName(name));

...
public class LagNotifier extends Module {
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (!rubberbandTimer.passedMs(5000) && rubberbandNotify.getValue()) {
            ...
            FontRenderers.modules.drawCenteredString(context.getMatrices(), (isPl() ? "Wykryto rubberband! " : "Rubberband detected! ") + decimalFormat.format((5000f - (float) rubberbandTimer.getTimeMs()) / 1000f), (float) mc.getWindow().getScaledWidth() / 2f, (float) mc.getWindow().getScaledHeight() / 3f, new Color(0xFFDF00).getRGB());
        }

        if (packetTimer.passedMs(responseTreshold.getValue() * 1000L) && serverResponseNotify.getValue()) {
            DecimalFormat decimalFormat = new DecimalFormat("#.#");
            FontRenderers.modules.drawCenteredString(context.getMatrices(), (isPl() ? "Serwer przestal odpowiadac! " : "The server stopped responding! ") + decimalFormat.format((float) packetTimer.getTimeMs() / 1000f), (float) mc.getWindow().getScaledWidth() / 2f, (float) mc.getWindow().getScaledHeight() / 3f, new Color(0xFFDF00).getRGB());
        ...
        if (Managers.SERVER.getTPS() < 10 && notifyTimer.passedMs(60000) && tpsNotify.getValue()) {
            String msg = isPl() ? "TPS serwera jest ponizej 10!" : "Server TPS is below 10!";
            if (ModuleManager.tpsSync.isDisabled()) msg += isPl() ? " Zaleca sie wlaczyc TPSSync" : "It is recommended to enable TPSSync";
        ...
        if (Managers.SERVER.getTPS() > 15 && isLagging) {
            Managers.NOTIFICATION.publicity("LagNotifier", isPl() ? "TPS serwera sie ustabilizowal!" : "Server TPS has stabilized!", 8, Notification.Type.SUCCESS);
            isLagging = false;
        }

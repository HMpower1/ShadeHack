...
public final class AutoAuth extends Module {
    ...
    public void onEnable() {
        String warningMsg = isPl() ?
                Formatting.RED + "PL! " + Formatting.RESET + "PL PL PL PL, PL PL PL " + Formatting.RED + " PL PL!" :
        ...
        if (event.getPacket() instanceof GameMessageS2CPacket pac && mc.getNetworkHandler() != null) {
            ...
            switch (mode.getValue()) {
                case Custom -> {
                    ...
                    if (password.isEmpty()) {
                        sendMessage(Formatting.RED + (isPl() ? "PL PL: PL PL!" : "Registration error: Password is empty!"));
                        return;
                    }
                }
            ...
            if (m.contains("/reg") || m.contains("/register") || m.contains("PL")) {
                mc.getNetworkHandler().sendChatCommand("reg " + password + " " + password);
                if (show.getValue()) sendMessage((isPl() ? "PL PL: " : "Your password: ") + Formatting.RED + password);
                Managers.NOTIFICATION.publicity("AutoAuth", isPl() ? "PL PL!" : "Registration completed!", 4, Notification.Type.SUCCESS);
            } else if (m.contains("PL") || m.contains("/l")) {
                mc.getNetworkHandler().sendChatCommand("login " + password);
                Managers.NOTIFICATION.publicity("AutoAuth", isPl() ? "PL PL!" : "Logged in!", 4, Notification.Type.SUCCESS);
            }
        }
    }
}

...
import shade.features.hud.impl.StaffBoard;
...
public class Spammer extends Module {
    ...
    public static void loadSpammer() {
        try {
            File file = new File(ConfigManager.CONFIG_FOLDER_NAME + "/misc/spammer.txt");
    ...
    private void changeFact() {
        Managers.ASYNC.run(() -> {
            try {
                ...
                disable(isPl() ? "Nie udalo sie pobrac faktu. Sprawdz polaczenie z internetem." : "Failed to load the fact, can you turn on the Internet?");
            }
        });
    }
    ...
    public void onUpdate() {
        if (timer_delay.passedMs((long) (delay.getValue() * 1000))) {
            ...
            if (messages.getValue() == Messages.File) {
                if (SpamList.isEmpty()) {
                    disable(isPl() ? "Plik spammer jest pusty!" : "The spammer file is empty!");
                    return;
                }

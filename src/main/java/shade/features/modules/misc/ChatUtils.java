...
import shade.injection.accesors.IGameMessageS2CPacket;
...
public class ChatUtils extends Module {
    ...
    Map<String, String> ruToEng = Collections.emptyMap();
        ...
        if (event.getPacket() instanceof GameMessageS2CPacket pac) {
            ...
            if (mention.getValue()) {
                if (pac.content.getString().contains(mc.player.getName().getString()) && messageTimer.passedMs(1000)) {
                    Managers.NOTIFICATION.publicity("ChatUtils", isPl() ? "PL PL PL PL!" : "You were mentioned in the chat!", 4, Notification.Type.WARNING);
            ...
            if (!pmSound.is(PMSound.Off) && (content.contains("whisper") || content.contains("-> PL") || content.contains("-> " + NameProtect.getCustomName()) || content.contains("-> me") || content.contains(" says:"))) {
    ...
    public void onPacketSend(PacketEvent.@NotNull Send e) {
        if (e.getPacket() instanceof ChatMessageC2SPacket pac) {
            if (antiCoordLeak.getValue() && pac.chatMessage.replaceAll("\\D", "").length() >= 6) {
                sendMessage("[ChatUtils] " + (isPl() ? "PL PL PL PL!" : "The message contains coordinates!"));
                e.cancel();
            }

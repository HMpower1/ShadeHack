...
import shade.features.cmd.impl.KitCommand;
...
public class AutoGear extends Module {
    ...
    public void setup() {
        ...
        if (selectedKit.isEmpty()) {
            disable(isPl() ? "PL wybrany PL! PL PL kit" : "No kit is selected! Use the kit command");
            return;
        }

        sendMessage(isPl() ? "PL PL => " + Formatting.AQUA + selectedKit : "Selected kit -> " + Formatting.AQUA + selectedKit);

        String kitItems = KitCommand.getKitItems(selectedKit);

        if (kitItems.isEmpty() || kitItems.split(" ").length != 36) {
            disable(isPl() ? "PL PL PL PL PL! PL PL PL" : "There was an error in the kit configuration! Create the kit again");
            return;
        }

...
public class DurabilityAlert extends Module {
            ...
            for (PlayerEntity player : mc.world.getPlayers()) {
                ...
                for (ItemStack stack : player.getInventory().armor) {
                    ...
                    if (getDurability(stack) < percent.getValue() && timer.passedMs(30000)) {
                        mc.player.networkHandler.sendChatCommand("msg " + player.getName().getString() + (isPl() ? " Pilnie napraw zbroje!" : " Fix your armor right now!"));
    ...
    public void onRender2D(DrawContext context) {
        if (need_alert) {
            FontRenderers.sf_bold.drawCenteredString(context.getMatrices(), isPl() ? "Pilnie napraw zbroje!" : "Fix your armor right now!", (float) mc.getWindow().getScaledWidth() / 2f, (float) mc.getWindow().getScaledHeight() / 3f, new Color(0xFFDF00).getRGB());

...
public class ElytraSwap extends Module {
    ...
    public void useFireWork() {
        ...
        if (hotbarFireWorkResult.found()) {
            ...
            sendMessage(isPl() ? "Nie masz fajerwerkow!" : "You've got no fireworks!");
            return;
        }
    ...
    private void swapChest(boolean disable) {
        ...
        if (mc.player.getInventory().getStack(38).getItem() == Items.ELYTRA) {
            ...
            if (slot != -1) {
                ...
                if (disable) disable(isPl() ? "Nie masz napierśnika!" : "You don't have a chestplate!");
                else sendMessage(isPl() ? "Nie masz napierśnika!" : "You don't have a chestplate!");
                return;
            }
            ...
            if (disable) disable(isPl() ? "Nie masz elytry!" : "You don't have an elytra!");
            else sendMessage(isPl() ? "Nie masz elytry!" : "You don't have an elytra!");
            return;
        }

        if (disable)
            disable(isPl() ? "Zamieniono! Wylaczam.." : "Swapped! Disabling..");
    }
}

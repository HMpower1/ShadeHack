...
public class Companion extends HudElement {
    ...
    public void onUpdate() {
        for (PlayerEntity player : mc.world.getPlayers()) {
            ...
            if (isPl())
                message = player.getName().getString() + " uzyl " + (Managers.COMBAT.popList.get(player.getName().getString()) > 1 ? Managers.COMBAT.popList.get(player.getName().getString()) + " totemow i zginal! PL" : "totemu i zginal! PL");
            else
    ...
    public void onTotemPop(@NotNull TotemPopEvent event) {
        ...
        if (isPl())
            message = event.getEntity().getName().getString() + " uzyl " + (event.getPops() > 1 ? event.getPops() + " totemow!" : "totemu!");
        else

...
import shade.gui.hud.HudEditorGui;
...
import shade.features.modules.combat.AutoAnchor;
...
import shade.utility.render.animation.EaseOutCirc;
...
public class TargetHud extends HudElement {
    ...
    private final Setting<ModeEn> Mode = new Setting<>("Mode", ModeEn.Shade);
    ...
    public void onEnable() {
        try {
            custom = ExoUtility.getCustomImg("thud");
        } catch (Exception e) {
            sendMessage(".minecraft -> " + ConfigManager.CONFIG_FOLDER_NAME + " -> misc -> images -> thud.png");
        }
    }
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (animation.getAnimationd() > 0) {
            ...
            switch (Mode.getValue()) {
                case Shade -> renderShade(context, health, animationFactor);
    ...
    private void renderShade(DrawContext context, float health, float animationFactor) {
        float hurtPercent = target.hurtTime / 6f;

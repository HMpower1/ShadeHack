...
import shade.features.cmd.args.MacroArgumentType;
...
public class MacroCommand extends Command {
    ...
    public void executeBuild(@NotNull LiteralArgumentBuilder<CommandSource> builder) {
        ...
        builder.then(literal("remove").then(arg("macro", MacroArgumentType.create()).executes(context -> {
            ...
            if (macro == null) {
                sendMessage(isPl() ? "Takie makro nie istnieje!" : "Wrong macro name!");
                return SINGLE_SUCCESS;
            }

            Managers.MACRO.removeMacro(macro);
            sendMessage((isPl() ? "Usunieto makro " : "Removed macro ") + macro.getName());
                        ...
                        .then(arg("bind", StringArgumentType.word())
                                .then(arg("args", StringArgumentType.greedyString()).executes(context -> {
                                    ...
                                    if (InputUtil.fromTranslationKey("key.keyboard." + bind.toLowerCase()).getCode() == -1) {
                                        sendMessage(isPl() ? "Niepoprawny bind!" : "Wrong bind!");
                                        return SINGLE_SUCCESS;
                                    }
                                    ...
                                    sendMessage(isPl() ? "Dodano makro " + name + " pod klawisz " + toString(macro.getBind()) : "Added macro " + name + " to " + toString(macro.getBind()));

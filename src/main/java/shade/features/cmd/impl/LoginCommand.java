...
public class LoginCommand extends Command {
    ...
    public void executeBuild(@NotNull LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(arg("name", StringArgumentType.word()).executes(context -> {
            ...
            sendMessage((isPl() ? "Konto zmieniono na: " : "Switched account to: ") + mc.getSession().getUsername());

            return SINGLE_SUCCESS;
        }));

        builder.executes(context -> {
            sendMessage(isPl() ? "Uzycie: .login <nickname>" : "Usage: .login <nickname>");
    ...
    public void login(String name) {
        try {
            ...
            sendMessage((isPl() ? "Niepoprawna nazwa! " : "Incorrect username! ") + exception);
        }
    }

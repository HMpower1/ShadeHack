    ...
    public void executeBuild(@NotNull LiteralArgumentBuilder<CommandSource> builder) {
        ...
        builder.then(literal("add").then(arg("item", ChestStealerArgumentType.create()).executes(context -> {
            ...
            if(result != null){
                ...
                sendMessage(Formatting.GREEN + blockName + (isPl() ? " dodany do InvCleaner" : " added to InvCleaner"));
            } else {
        ...
        builder.then(literal("del").then(arg("item", ChestStealerArgumentType.create()).executes(context -> {
            ...
            if(result != null){
                ...
                sendMessage(Formatting.GREEN + blockName + (isPl() ? " usuniety z InvCleaner" : " removed from InvCleaner"));
            } else {
                sendMessage(Formatting.RED + (isPl() ? "Nie ma takiego przedmiotu!" : "There is no such item!"));
            }
            return SINGLE_SUCCESS;

    ...
    public void executeBuild(@NotNull LiteralArgumentBuilder<CommandSource> builder) {
        ...
        builder.then(arg("x", IntegerArgumentType.integer())
                .then(arg("z", IntegerArgumentType.integer()).executes(context -> {
                    ...
                    Shade.gps_position = new BlockPos(x, 0, z);

                    sendMessage("GPS ustawiony na X: " + Shade.gps_position.getX() + " Z: " + Shade.gps_position.getZ());
                    return SINGLE_SUCCESS;
                })));
        builder.executes(context -> {
            sendMessage("Sprobuj .gps off / .gps x z");
            return SINGLE_SUCCESS;
        });
    }
}

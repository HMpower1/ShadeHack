    ...
    public void executeBuild(@NotNull LiteralArgumentBuilder<CommandSource> builder) {
        builder.then(literal("down").executes(context -> {
            ...
            for (i = 1; i < 255; ++i) {
                ...
                sendMessage(Formatting.RED + (isPl() ? "Nie ma gdzie sie przeniesc!" : "There's nowhere to clip!"));
        ...
        builder.then(arg("count", DoubleArgumentType.doubleArg()).executes(context -> {
            ...
            try {
                sendMessage(Formatting.GREEN + "Przenosze o " + count + " blokow");
                clip(count);
        ...
        builder.executes(context -> {
            sendMessage("Sprobuj .vclip <liczba>");
            return SINGLE_SUCCESS;
        });
    }
